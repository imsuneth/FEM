...
class Element:
    ...
    def analyze(self, tolerance):  # for the first iteration set the initial call to True
        ...
        returnMatrix = np.transpose(self.rotMatrix()) @ np.transpose(self.rigidBodyTransMatrix()) @ self.K_element @ self.rigidBodyTransMatrix() @ self.rotMatrix()  # 6x6 matrix refering global co-ordinate system
        print("Element %d element matrix"%self.id)
        print(returnMatrix)
        return returnMatrix

    def conditionCheck(self,mat1,value):
        if mat1.max()>value:
            return True
        else:
